...
- **Banned in `packages/engine`** (ESLint `no-restricted-*`): `Date.now`, `new Date()` (argless), `Math.random`, `String.prototype.localeCompare`, `Intl.*` (ICU varies by platform/version), and transcendental float math (`Math.exp/pow/log/sin…`) in any state-affecting path — use integer/fixed-point forms or precomputed tables.
...
- **Concurrency:** LLM I/O is the only concurrency; completion order never matters because results are re-sorted by agentId before application (ADR-0007).
