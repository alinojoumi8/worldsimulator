...
- Lagging clients receive `{fromSeq,toSeq}` in a `gap` frame and invalidate the simulation, status, list, and event-ledger queries. Compression buffering is disabled; `:hb` defaults to 15 seconds.
