...
- **Simulation capability model (b):** every future intent carries an `actor`; validators must check self-actions, ownership, household scope, and offices held. This remains domain logic, not HTTP auth, and is the defense against illegal LLM-proposed actions (INV-10).
...
- Capability checks become hot-path validation code with dedicated tests as actor modules arrive.
