...
1. create a mock-mode Riverbend simulation with seed 42 and end tick 31;
2. start and pause the run through the cockpit controls;
3. open the citizen directory and inspect a citizen's employment and finances;
4. open the credit explorer, select a real seeded loan, and verify its stored
   why-panel and evidence;
5. return to the cockpit and schedule an approved 30% fuel-price shock;
6. resume to completion and observe the authoritative CPI series through tick
   31.
...
bounds, requires a paused run, and displays the scheduled tick from the typed
receipt. The engine still validates every request and applies it only at a
committed tick boundary.
...
needs no provider credentials and cannot incur live-provider work.
...
- component tests for the exact approved event request shapes, paused-run
  authority boundary, and typed scheduling receipt;
- one real Chromium test covering create, lifecycle controls, citizen and loan
  explanation, event scheduling, tick-boundary application, and CPI rendering;
