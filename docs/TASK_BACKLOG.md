...
**Status:** implemented. Exact bigint priced-round rules reject fractional-cent prices, fractional shares, inconsistent negotiated equity, and signed-64-bit overflow before IDs or state are consumed. Migration 33 adds fund cash-account links, generalized authoritative cap tables and immutable ownership stakes, the investment contract template, and immutable completed investments guarded by matching proposal, cash, deployment, contract, event, and share identities. Closing atomically performs the signed contract, ROW capital call for only the fund shortfall, domestic fund-to-company transfer, fund deployment, evented share issuance, cap-table revision, proposal completion, and full pre/post cap-table diff. Logical state-hash v25, INV-4, tamper guards, populated upgrade, rollback/reopen, deferred event ordering, and snapshot restore-equivalence coverage are green. See [WS-803 evidence](WS_803_CAP_TABLE_CLOSING.md).
